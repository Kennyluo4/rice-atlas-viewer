...
if (input_1_users_select_cate != 'ACR'){
  ...
  plot.act.scores    <- function(meta_fl,output_dir,
    ...
    for (i in 1:length(gids)){
      ...
      # plot
      p <- plot(df2[[umap1_colnm]], df2[[umap2_colnm]], col=colvec,
      ...
      p <- p + add.color.bar(3, cols, title='low -> high',subtitle='', lims=NULL,prompt=F,x=min(df2[[umap1_colnm]]+8),y=min(df2[[umap2_colnm]])+1,cex=0.15)
